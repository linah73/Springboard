...
SELECT firstname, surname FROM `Members` WHERE memid = 36 ORDER BY joindate;

/* Q7: Produce a list of all members who have used a tennis court.
Include in your output the name of the court, and the name of the member
formatted as a single column. Ensure no duplicate data, and order by
the member name. */
SELECT DISTINCT f.name as Facility, CONCAT_WS(" ", m.firstname, m.surname) as Member
FROM `Bookings` as b
	INNER JOIN `Facilities` as f USING(facid)
	INNER JOIN `Members` as m USING(memid)
WHERE f.name IN ('Tennis Court 1', 'Tennis Court 2')  AND memid != 0
ORDER BY Member;
...
AND ((memid != 0 AND f.membercost * b.slots > 30) OR (memid = 0 AND f.guestcost * b.slots > 30))
ORDER BY Cost DESC;
...
SELECT m.firstname, m.surname, x.firstname ||' '|| x.surname AS recommendedbyName
FROM Members m
LEFT JOIN Members x ON m.recommendedby = x.memid
WHERE x.memid != 0


/* Q12: Find the facilities with their usage by member, but not guests */
SELECT f.name as Facility,  m.firstname ||' '|| m.surname as Member,
	SUM(b.slots) as Facility_Usage
FROM `Bookings` as b
	INNER JOIN `Facilities` as f USING(facid)
	INNER JOIN `Members` as m USING(memid)
...
SELECT f.name as Facility, strftime('%m', b.starttime) as Month,
	SUM(b.slots) as Facility_Usage
FROM `Bookings` as b
	INNER JOIN `Facilities` as f USING(facid)
	INNER JOIN `Members` as m USING(memid)
